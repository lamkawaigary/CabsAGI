  ...
  useEffect(() => {
    ...
    const unsub = onAuthStateChanged(auth, async (fbUser) => {
      ...
      try {
        ...
        // Firestore failed - DON'T default to passenger!
        // Try to determine role from email as a fallback
        const email = fbUser.email || ''
        const isDriverEmail = email.toLowerCase().includes('driver') || email.toLowerCase().includes('garylkw')
        ...
        setCurrentUser({
          ...
          phoneVerified: false,
          email: fallbackEmail,
          role: isDriverEmail ? 'driver' : 'passenger', // Use email hint (not reliable!)
          points: 0,
          kycStatus: 'n/a',
          driverApproved: false,
          kycSubmittedAt: null,
