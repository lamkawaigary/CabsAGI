...
export default function PassengerHome() {
  ...
  const placeOrderNow = async (usedQuote: QuoteView) => {
    ...
    navigate('/orders', {
      state: {
        notice: {
          text: '訂單已建立，已同步到你的 Firebase 訂單列表。',
          tone: 'ok',
        },
      },
    })
        ...
        {quote && routeInfo && (
          <div style={{ borderRadius: 12, background: '#f5f9f6', border: '1px solid #dde8df', padding: 12, display: 'grid', gap: 6 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14 }}>
              ...
              <strong>{quote.duration} 分鐘</strong>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14 }}>
              <span style={{ color: '#5c7068' }}>附加費</span>
              <strong>HK${quote.tollsTotal}</strong>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
              <span style={{ color: '#5c7068' }}>路徑來源</span>
              <strong>{routeInfo.hasRealPath ? 'Tencent Driving API' : 'Fallback 估算'}</strong>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 18, borderTop: '1px solid #d6e3da', paddingTop: 8 }}>
              <span style={{ color: '#1f3f38', fontWeight: 700 }}>預估總價</span>
              <strong style={{ color: '#1e4f43' }}>HK${quote.total}</strong>
            </div>
